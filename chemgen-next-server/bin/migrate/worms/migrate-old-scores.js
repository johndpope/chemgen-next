...
var sdk_1 = require("../../../common/types/sdk");
...
// let defaultUserId: 9;
var expAssays = path.resolve(path.join(__dirname, 'rnai', 'data', 'scores', 'Experiment_Assay.csv'));
var expPlates = path.resolve(path.join(__dirname, 'rnai', 'data', 'scores', 'Experiment_ExperimentPlate.csv'));
var manualScores = path.resolve(path.join(__dirname, 'rnai', 'data', 'scores', 'Experiment_ManualScores.csv'));
//@ts-ignore
...
function getNewAssayIds() {
    return new Promise(function (resolve, reject) {
        //@ts-ignore
        Promise.map(data.expAssay.data, function (expAssay) {
            var expPlate = lodash_1.find(data.expPlates.data, { experimentPlateId: expAssay.plateId });
...
function mapManualScores() {
    var mappedManualScores = data.manualScores.data.map(function (manualScore) {
        var mappedResult = lodash_1.find(mappedData, { oldAssayId: manualScore.assayId });
        var userId, userName;
        try {
            // @ts-ignore
        ...
        var manualscoreGroup = '';
        var manualscoreCode = '';
        if (lodash_1.isEqual(manualScore.manualscoreCode, 'M_NO_EFFECT')) {
            manualScore.scoreCodeId = 67;
        }
        var code = lodash_1.find(manualScoreCodes, function (manualScoreCode) {
            return lodash_1.isEqual(Number(manualScore.scoreCodeId), Number(manualScoreCode.manualscorecodeId));
        });
        if (!code) {
            ...
            if (mappedResult) {
                var newManualScore = new sdk_1.ExpManualScoresResultSet({
    ...
    mappedData.map(function (thing) {
        ...
        var firstPassScore = new sdk_1.ExpManualScoresResultSet({
            ...
            timestamp: lodash_1.find(mappedManualScores, { treatmentGroupId: expAssay2reagent.treatmentGroupId }).timestamp
        });
        var hasManualScoreScore = new sdk_1.ExpManualScoresResultSet({
        ...
        if (!fs.existsSync(data[dataKey].file)) {
            ...
            Papa.parse(fs.createReadStream(data[dataKey].file), {
                ...
                step: function (results) {
                    Object.keys(results.data[0]).map(function (key) {
                        results.data[0][lodash_1.camelCase(key)] = results.data[0][key];
                        if (!lodash_1.isEqual(key, lodash_1.camelCase(key))) {
...
function filterExpAssays() {
    var assayIds = lodash_1.uniq(data.manualScores.data.map(function (expManualScore) {
        return expManualScore.assayId;
    }));
    data.expAssay.data = assayIds.map(function (assayId) {
        return lodash_1.find(data.expAssay.data, { assayId: assayId });
    });
    var expAssayCSV = Papa.unparse(data.expAssay.data);
    fs.writeFileSync(data.expAssay.file, expAssayCSV);
    return data;
}
//# sourceMappingURL=migrate-old-scores.js.map