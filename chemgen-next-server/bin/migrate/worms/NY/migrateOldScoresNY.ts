...
import {EegiResults} from "./parse_eegi_denorm";
...
function getNewAssayIds() {
  return new Promise((resolve, reject) => {
    let pageSize = 1000;
    let totalPages = Math.round(data.expAssay.data.length / pageSize);
    ...
    copiedData.expAssay.data = data.expAssay.data.map((eegiResult) => {
      return eegiResult;
    });
    console.log('finished copying data');
    //@ts-ignore
    Promise.map(range(0, totalPages + 1), (page: number) => {
      let end = Number(page) + Number(pageSize);
      ...
      //@ts-ignore
      return Promise.map(data.expAssay.data, (eegiResult: EegiResults) => {
...
function mapManualScores() {
  ...
  let mappedManualScores = [];
  data.expAssay.data.map((eegiResult: EegiResults) => {
    let mappedResult = find(mappedData, {oldAssayId: eegiResult['experiment.id']});
    let userObj, userId, userName;
    try {
      // @ts-ignore
      // userId = users[Number(manualScore.scorerId)].userId || defaultUserId;
      userObj = find(data.eegiUsers.data, {id: String(eegiResult['manualscore.scorer_id'])});
      //@ts-ignore
    ...
    if (has(eegiResult, ['manualscore.id'])) {

      if (isEqual(eegiResult['manualscorecode.id'], '0')) {
        eegiResult['manualscorecode.id'] = 67;
      }

      let code = find(manualScoreCodes, (manualScoreCode) => {
        return isEqual(Number(eegiResult['manualscorecode.id']), Number(manualScoreCode.manualscorecodeId));
      });
      if (!code) {
    ...
    if (get(eegiResult, 'experiment.is_junk')) {
      if (get(eegiResult, 'experiment.is_junk')) {
...
function addFirstPass(mappedResult, mappedManualScores, eegiResult) {
  let expAssay2reagent = mappedResult.expAssay2reagent;
  let firstPassScore = new ExpManualScoresResultSet({
    'manualscoreGroup': 'FIRST_PASS',
    'manualscoreCode': 'FIRST_PASS_INTERESTING',
    'manualscoreValue': 1,
...
function readInData() {
  return new Promise((resolve, reject) => {
    //@ts-ignore
    return Promise.map(Object.keys(data), (dataKey: string) => {
...
function filterExpAssays() {
  data.expAssay.data = data.expAssay.data.filter((eegiResult) => {
    let junk = Number(eegiResult['experiment.is_junk']);
    return has(eegiResult, ['manualscore.id']) || junk;
  });
