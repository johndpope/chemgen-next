...
RnaiLibrary.helpers.primary.buildControlTag = function (workflowData, contentObj) {
    var cnTag = "_C-Restrictive_WS-" + workflowData.controlStrain;
    var cmTag = "_C-Restrictive_WS-" + workflowData.mutantStrain;
    /**
     Restrictive N2
     **/
...
RnaiLibrary.helpers.secondary.buildControlTag = function (workflowData, contentObj) {
    var cnTag = "_C-Restrictive_WS-" + workflowData.controlStrain;
    var cmTag = "_C-Restrictive_WS-" + workflowData.mutantStrain;
    var strain = RnaiLibrary.helpers.wormStrain(contentObj.barcode);
...
RnaiLibrary.helpers.buildControlTags = function (workflowData, contentObj) {
    ...
    contentObj = RnaiLibrary.helpers[workflowData.screenStage]['buildControlTags'](workflowData, contentObj);
    return contentObj;
};
 ...
 */
RnaiLibrary.helpers.parseCond = function (barcode) {
    if (barcode.match('E')) {
        return 'Permissive';
    }
 ...
 * @returns {number}
 */
RnaiLibrary.helpers.isDuplicate = function (barcode) {
    if (barcode.match('D')) {
        return 1;
    }
    else {
 ...
 * @returns {any}
 */
RnaiLibrary.helpers.getTemp = function (barcode, workflowData) {
    var cond = RnaiLibrary.helpers.parseCond(barcode);
    if (cond === 'Permissive') {
...
RnaiLibrary.search = function (where) {
    RnaiLibrary.find({
        where: where
    })
 ...
 */
RnaiLibrary.helpers.checkLibraryResult = function (libraryResult) {
    if (!libraryResult) {
        libraryResult = {};
