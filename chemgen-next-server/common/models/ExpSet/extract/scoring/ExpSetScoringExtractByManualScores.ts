 ...
 * @param search
 */
ExpSet.extract.workflows.getScoredExpSets = function (search: ExpSetSearch) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let sqlQuery = ExpSet.extract.buildNativeQueryGetDetailedScores(data, search, search.scoresExist);
 ...
 */
ExpSet.extract.buildNativeQueryGetDetailedScores = function (data: ExpSetSearchResults, search: ExpSetSearch, hasManualScores: Boolean) {

  let query = knex('exp_assay2reagent');
  query = query
    .distinct('treatment_group_id')
    .whereNot({treatment_group_id: null})
...
/**
 * WIP - Create more complex queries on the scores
 */
